<?php
    ...
    //-------------Verifca se o usuário já registrou os seus dados pessoais para pular etapa de cadastro!(INICIO)------------------------//
    $sql = mysqli_query($mysqli, "SELECT NOME_COMPLETO, DATA_DE_NASCIMENTO, GENÊRO, TELEFONE, UF, CIDADE, BAIRRO, CEP, RUA, NUM_CASA, Termos_Condições FROM usuarios WHERE ID_USUARIO = '$id'");
    $result = mysqli_fetch_assoc($sql);
    $result['NOME_COMPLETO'];
    if($result['NOME_COMPLETO'] !=""){

        header("Location: /cepet/usuario/adotar.php?ID_PET=$_GET[ID_PET]");
    }
      //-------------Verifca se o usuário já registrou os seus dados pessoais para pular etapa de cadastro!(FIM------------------------//
...
<form action="config/cadastra_usuario2.php" method="POST">
<!-------------------------------------------------------------------------------------->    
 ...
 <!-- Não apagar "name" nos campos de input ou select. PHP precisa dos names para puxar as informações para as variaveis-->
    ...
    <p>Nome Completo</p>
        <input type="text" name="nome" id="usuarionome" maxlength="50"  placeholder="Digite seu nome completo" required>

        <p>CPF</p>
        <input oninput="mascara(this)" type="text" name="usuariocpf" id="usuariocpf" placeholder="Digite seu CPF" max length="11" required>

        <p>Gênero</p>
        <select name="genero" id="usuariogenero" required>
        ...
        <input type="date" name="data_nasc" id="usuarionascimento" required>
        ...
        <p>Estado</p>
        <select name="estado" id="usuarioestado" required>
